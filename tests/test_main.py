...
from truenas_unlock import Config, Dataset, SecretsMode, app, run_unlock, run_lock, run_status
...
@pytest.mark.anyio
async def test_run_unlock_success(mock_config: Config, mock_client_cls) -> None:
...
@pytest.mark.anyio
async def test_run_unlock_connection_error(mock_config: Config, mock_client_cls) -> None:
...
@pytest.mark.anyio
async def test_run_unlock_exception(mock_config: Config, mock_client_cls) -> None:
...
@pytest.mark.anyio
async def test_run_lock(mock_config: Config, mock_client_cls) -> None:
    """Test run_lock logic."""
    mock_instance = mock_client_cls.return_value
...
@pytest.mark.anyio
async def test_run_status(mock_config: Config, mock_client_cls) -> None:
...
def test_cli_with_config(tmp_path, mock_client_cls):
...
def test_cli_daemon_mode(tmp_path, mock_client_cls):
